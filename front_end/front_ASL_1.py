...
#    IMAGE_FOLDER = "asl"
    ...
    with camera_col:
        # Camera input
        camera_image = st.camera_input("Take a picture", key=f"camera_1_{st.session_state.page_key['Home Page']}", label_visibility="collapsed")
...
#            st.markdown(f"You are giving up on {game_word}!")
        ...
        if st.session_state.challenge_completed:
            ...
            #st.session_state.user_gives_up = False
            #st.session_state.challenge_completed = False
            #st.session_state.cleanup_ready = False
